...
contract veAAHCore is OwnableUpgradeable, ReentrancyGuardUpgradeable, LPTokenWrapper {
    ...
    //         if (_node.balance > 0) {
    //             _var.rewardPerToken = _var.rewardPerToken.add(_var.lastSettledTime.sub(_var.lastUpdateTime).mul(_var.rewardRate).rdiv(_var.totalSupply.sub(_var.accSettledBalance)));

    //             //After the rewardpertoken is settled, add the balance of this node to accsettledbalance
    //             _var.accSettledBalance = _var.accSettledBalance.add(_node.balance);
    ...
    //             _var.lastSettledTime = MIN_STEP.sub(_now.sub(_var.lastSettledTime).mod(MIN_STEP)).add(_now);
    //             break;
    //         }
    ...
    //         ? _var.rewardPerToken
    //         : _var.rewardPerToken.add(_now.sub(_var.lastUpdateTime).mul(_var.rewardRate).rdiv(_var.totalSupply.sub(_var.accSettledBalance)));
    //     lastUpdateTime = _now;
    // }
